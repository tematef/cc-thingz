...
# empty config exits 127 with marker so the orchestrator skips the phase cleanly
assert_exit_stderr "empty cmd exits 127 with marker" 127 \
    "run-external-review: external_review_cmd is not set" \
    env PATH="$BIN_DIR:$PATH" bash "$REVIEW_SCRIPT" "" "review the whole diff"

# a whitespace-only config exits 127 with marker too
assert_exit_stderr "whitespace-only cmd exits 127 with marker" 127 \
    "run-external-review: external_review_cmd is not set" \
    env PATH="$BIN_DIR:$PATH" bash "$REVIEW_SCRIPT" "   " "review the whole diff"

# unconfigured token exits 127 with marker
assert_exit_stderr "unconfigured token exits 127 with marker" 127 \
    "run-external-review: external_review_cmd is not set" \
    env PATH="$BIN_DIR:$PATH" bash "$REVIEW_SCRIPT" '${user_config.external_review_cmd}' "review the whole diff"
...
assert_output "project copy destination" ".agents/exec-plan/prompts/review.md" "$dest"

if diff -q "$WORK_DIR/.agents/exec-plan/prompts/review.md" \
...
echo "MY OVERRIDE" > "$WORK_DIR/.agents/exec-plan/prompts/review.md"
...
mkdir -p "$WORK_DIR/fresh/.agents/exec-plan/agents"
echo "PROJECT LEVEL" > "$WORK_DIR/fresh/.agents/exec-plan/agents/quality.txt"
out=$(cd "$WORK_DIR/fresh" && bash "$EXEC_SCRIPTS/resolve-file.sh" agents/quality.txt "$WORK_DIR/data") || out="<rc=$?>"
...
mkdir -p "$WORK_DIR/link/.agents/exec-plan/prompts" "$WORK_DIR/outside"
ln -s "$WORK_DIR/outside/HIJACKED" "$WORK_DIR/link/.agents/exec-plan/prompts/review.md"
...
mkdir -p "$WORK_DIR/parentlink/.agents/exec-plan" "$WORK_DIR/parent-outside"
ln -s "$WORK_DIR/parent-outside" "$WORK_DIR/parentlink/.agents/exec-plan/prompts"
...
if [ -e "$WORK_DIR/.agents/exec-plan/prompts/task.md" ]; then
...
if [ -e "$WORK_DIR/.agents/exec-plan/prompts/finalizer.md" ]; then
...
if [ -e "$WORK_DIR/fresh/.agents/references/prompts/review.md" ] || [ -e "$WORK_DIR/fresh/.agents/exec-plan/../references/prompts/review.md" ]; then
