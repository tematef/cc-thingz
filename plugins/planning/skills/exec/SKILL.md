...
**hg skip**: Detect VCS with `vcs=$(bash ~/.gemini/config/plugins/planning/skills/exec/scripts/detect-vcs.sh)`. If `vcs` is `hg`, skip this entire step. Report to user: "hg detected — skipping external review (git-only). Override `prompts/codex-review.md` via `.agents/exec-plan/` (or `.claude/exec-plan/`) to enable hg-native review." Proceed directly to step 10.
...
- Subagents run with NO human available — they must NEVER ask the user a question (no ask_question, no pausing for input). They decide judgment calls the plan does not settle from the project's lint rules, GEMINI.md / AGENTS.md (or CLAUDE.md), and code conventions, and log each as a `[decision]`/`[deviation]` line for the completion report
