...
- **show rules**: run `bash ~/.gemini/config/plugins/brainstorm/scripts/resolve-rules.sh brainstorm-rules.md ~/.gemini/config/plugins_data/cc-thingz` and display the output. if the output is empty, tell the user no custom rules are configured at either level. otherwise, to determine the source, check if `.agents/brainstorm-rules.md` exists and is non-empty (project-level) — if not, the output came from user-level. tell the user which level it came from.
- **add/update project rules**: write content to `.agents/brainstorm-rules.md` in the current working directory.
